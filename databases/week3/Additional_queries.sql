...
SELECT meal.id as meal_id, title, discription, location, price, max_reservations, 
	IFNULL(SUM(number_of_guests), 0) AS total_reservations, 
    IFNULL((max_reservations - SUM(number_of_guests)), max_reservations) as available_spots
FROM meal
...
select meal.id, review.title, review.discription,review.meal_id, review.stars
...
select meal.id, review.title, review.discription,review.meal_id, CAST(ROUND(AVG(stars),2) AS DEC(10,2))
    avg_number_of_stars
from meal inner join review on meal.id = review.meal_id
GROUP BY
    meal.title
